...
; Date:      2017-08-19
; Recent:    2018-04-04
; Original: https://www.autohotkey.com/boards/viewtopic.php?t=36047
; Modified by KR1ZZ1
Class Vis2 {
    class OCR extends Vis2.functor {
        call(self, image:="", language:="", options:="", tsv:=""){
            return (image != "") ? (new Vis2.provider.Tesseract()).OCR(image, language, options, tsv)
    ...
    class provider {
        class Tesseract {
            ...
            fileConvertedText := A_Temp "\Vis2_text" this.uuid

            __New(language:=""){
                this.language := language
                }
            OCR(image, language:="", options:="", tsv:=""){
                this.language := language
                this.fileConvertedText .= (tsv) ? ".tsv":".txt"
                try {
                    screenshot := Vis2.stdlib.toFile(image, this.file, options)
                    this.preprocess(screenshot, this.fileProcessedImage)
                    this.convert(this.fileProcessedImage, this.fileConvertedText, tsv)
            ...
            convert(in:="", out:="", tsv:=0){
                ...
                _cmd .= q this.tesseract q " --tessdata-dir " q this.tessdata_best q " " q in q " " q SubStr(out, 1, -4) q
